...
# Block device created by phram
rofsdev="/dev/mtdblock0"
...
while [ $count -lt 5 ]; do
    ...
    echo "waiting for $rwfsdev"
    sleep 1
    count=$((count + 1))
done

# XXX: Disable this feature to reduce the size of initramfs by removing gptfdisk and parted
...
mkdir -p $rodir
if ! mount $rofsdev $rodir -t squashfs -o ro; then
    /bin/sh
fi
...
# XXX: Disable this feature to reduce the size of initramfs by removing libgpiod-tools
